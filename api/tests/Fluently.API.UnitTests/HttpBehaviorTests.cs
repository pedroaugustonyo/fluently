...
public sealed class HttpBehaviorTests
{
    ...
    public async Task ExceptionHandler_KnownApiException_WritesEnglishStatusTitle()
    {
        ...
        var handler = new ApiExceptionHandlerMiddleware(NullLogger<ApiExceptionHandlerMiddleware>.Instance);
        ...
        Assert.Equal("Esta questão já foi respondida.", problem.RootElement.GetProperty("detail").GetString());
        Assert.Equal(context.TraceIdentifier, problem.RootElement.GetProperty("traceId").GetString());
    }
